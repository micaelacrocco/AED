...
public class ManejadorArchivosGenerico2 {
	/**
	...
	public static String[] leerArchivoXPalabras(String nombreCompletoArchivo) {
		FileReader fr;
		ArrayList<String> listaPalabrasArchivo = new ArrayList<String>();
		try {
			fr = new FileReader(nombreCompletoArchivo);
			BufferedReader br = new BufferedReader(fr);
			String lineaActual = br.readLine();
			while (lineaActual != null) {
				...
				for (String palabra : palabras) {
					listaPalabrasArchivo.add(palabra);
				}
				lineaActual = br.readLine();
			}
			br.close();
		...
		return listaPalabrasArchivo.toArray(new String[0]);
	}
}
