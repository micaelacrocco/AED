...
public class TClasificador {
    ...
    public static void main(String[] args) {
        ...
        int[][] ordenes = {
            generarDatosAscendentes(300),
            generarDatosDescendentes(300),
            generarDatosAleatorios(300)
        };
        ...
        for (int tamanio : tamanios) {
            for (int i = 0; i < 3; i++) {
                int[] datos = Arrays.copyOf(ordenes[i], tamanio);

                long startTime = System.nanoTime();
                clasificador.ordenarPorQuickSort(datos);
                long endTime = System.nanoTime();

                long duration = (endTime - startTime) / 1000000; // Milliseconds
                System.out.println("Tamaño: " + tamanio + ", Orden inicial: " + getOrdenInicial(i) + ", Tiempo de ejecución: " + duration + " ms");
            }
        }
    }

    private static String getOrdenInicial(int i) {
        switch (i) {
            case 0: return "Ascendente";
    ...
    private static int[] generarDatosAscendentes(int tamanio) {
    ...
    private static int[] generarDatosDescendentes(int tamanio) {
        int[] datos = new int[tamanio];
        for (int i = 0; i < tamanio; i++) {
            datos[i] = tamanio - i;
        }
    ...
    private static int[] generarDatosAleatorios(int tamanio) {
        ...
        int[] datos = new int[tamanio];
        for (int i = 0; i < tamanio; i++) {
            datos[i] = random.nextInt(tamanio);
        }
        return datos;
    }
}
